...
export default async function handler(req, res) {
  ...
  const ladderRows = (steps, reps) => steps.map((w, i) => {
    const isTop     = i === steps.length - 1;
    const isBot     = i === 0;
    const bg        = isTop ? '#1A1A1A' : isBot ? '#F0F0F0' : '#FAFAFA';
    const setColor  = isTop ? '#aaa' : isBot ? '#999' : '#bbb';
    const repsColor = isTop ? '#888' : isBot ? '#999' : '#bbb';
    ...
    return `
      <tr style="background:${bg};">
        ...
        <td style="padding:13px 8px;font-size:22px;font-weight:900;color:${wColor};">${fmt(w)}<span style="font-size:12px;color:${unitColor};margin-left:3px;font-weight:400;">${data.unit}</span></td>
      ...
      <tr><td colspan="4" style="padding:0;">
        <table width="100%" cellpadding="0" cellspacing="0"><tr>
          ...
          <td style="padding:14px 16px;background:#F5F5F5;text-align:right;vertical-align:middle;">
            <span style="font-size:12px;color:#999;font-style:italic;white-space:nowrap;">${week.pctLabel}</span>
          </td>
        </tr></table>
      </td></tr>
      <tr><td colspan="4" style="padding:9px 16px;background:#EFEFEF;font-size:13px;color:#666;">
        Top set: <strong style="color:#1A1A1A;">${fmt(week.topSet)}${data.unit}</strong> &nbsp;·&nbsp; ${week.sets} sets
      </td></tr>
      ${ladderRows(week.ladder, week.reps)}
    </table>`;

  const liftGroupHTML = (lifts, groupLabel, limitingName, allOnTarget) => `
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:12px;border:1.5px solid #E8E8E8;border-radius:8px;overflow:hidden;">
      ...
      ${lifts.map(lift => {
        ...
        <tr style="background:${rowBg};">
          ...
          <td style="padding:12px 8px;font-size:15px;font-weight:900;color:${textColor};text-align:right;white-space:nowrap;">${fmt(lift.e1rm)}<span style="font-size:10px;color:${isLimiting ? '#e88' : '#bbb'};margin-left:2px;font-weight:400;">${data.unit}</span></td>
  ...
  const upperLifts = (data.llResults || []).filter(r => r.group === 'upper');
  const lowerLifts = (data.llResults || []).filter(r => r.group === 'lower');
...
<table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;width:100%;">
  ...
  <tr><td style="background:#111;padding:12px 40px;">
    <span style="font-size:11px;color:#666;letter-spacing:1px;">${data.exercise ? data.exercise + ' &nbsp;·&nbsp; ' : ''}${fmt(data.weight)}${data.unit} × ${data.topReps} reps &nbsp;·&nbsp; Micro plates: ${data.micro ? 'Yes' : 'No'}</span>
  </td></tr>
  ...
  <tr><td style="background:#fff;padding:32px 40px 24px;border-bottom:1px solid #EFEFEF;">
    ...
    <div style="font-size:64px;font-weight:900;color:#1A1A1A;line-height:1;letter-spacing:1px;">${fmt(data.e1rm)}<span style="font-size:22px;color:#ccc;margin-left:6px;font-weight:400;">${data.unit}</span></div>
  </td></tr>

  ${data.esRepMax ? `
  <tr><td style="background:#fff;padding:24px 40px;border-bottom:1px solid #EFEFEF;">
    <div style="font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#bbb;font-weight:700;margin-bottom:6px;">Estimated ${data.targetReps}-Rep Max</div>
    <div style="font-size:64px;font-weight:900;color:#1A1A1A;line-height:1;letter-spacing:1px;">${fmt(data.esRepMax)}<span style="font-size:22px;color:#ccc;margin-left:6px;font-weight:400;">${data.unit}</span></div>
  </td></tr>` : ''}

  ${data.ladder && data.ladder.length > 0 ? `
  <tr><td style="background:#fff;padding:24px 40px;border-bottom:1px solid #EFEFEF;">
    <div style="font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#bbb;font-weight:700;margin-bottom:16px;">Step Loading — ${data.ladder.length} Sets × ${data.targetReps} Reps</div>
    <table width="100%" cellpadding="0" cellspacing="0">
      <col width="70"/><col width="80"/><col/><col width="80"/>
      ${ladderRows(data.ladder, data.targetReps)}
    </table>
  </td></tr>` : ''}
  ...
  <tr><td style="background:#fff;padding:24px 40px;border-bottom:1px solid #EFEFEF;">
    ...
    ${data.weeks.map(w => weekCard(w)).join('')}
  </td></tr>` : ''}
  ...
  <tr><td style="background:#fff;padding:24px 40px;">
    ...
    ${upperHasData ? liftGroupHTML(upperLifts, 'Upper Body', upperLimiting ? upperLimiting.label : '', !upperLimiting) : ''}
    ${lowerHasData ? liftGroupHTML(lowerLifts, 'Lower Body', lowerLimiting ? lowerLimiting.label : '', !lowerLimiting) : ''}
  </td></tr>` : ''}
